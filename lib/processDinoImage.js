...
async function processDinoImage(buffer) {
  ...
  if (bestIdx >= 0) {
    ...
    // Keep full image dimensions (don't crop) so the texture aligns with the
    // species PNG outline that the client uses for UV mapping.
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (mask.ucharAt(y, x) > 0) {
          masked[idx] = data[idx];
          masked[idx + 1] = data[idx + 1];
          masked[idx + 2] = data[idx + 2];
          masked[idx + 3] = data[idx + 3];
        } else {
          masked[idx + 3] = 0; // transparent
        }
      }
    }
    ...
    mask.delete();
    outData = masked;
    // outWidth/outHeight stay as full image dimensions
  }
