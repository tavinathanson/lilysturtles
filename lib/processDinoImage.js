...
2. The dot-free region contains the dinosaur drawing. Give the tight bounding box of JUST the dinosaur outline and coloring, as fractions of image width/height. Exclude dots, title text, indicator dots, and whitespace.
Respond ONLY with JSON: {"species":"trex","bbox":{"x":0.1,"y":0.2,"w":0.6,"h":0.7}}`
          }
        ]
      }]
    });
...
async function processDinoImage(buffer) {
  ...
  // ── Step 5: Detect species + bbox from ORIGINAL photo (Sonnet reads title) ──
  const originalPng = await sharp(data, { raw: { width, height, channels: 4 } })
    .resize(800, 800, { fit: 'inside' }).png().toBuffer();
  const { species, bbox } = await detectDino(originalPng);
