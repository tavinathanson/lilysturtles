...
// Original species PNG files — the texture must match these dimensions
// so UVs on the 3D model align correctly.
const SPECIES_PNGS = {
...
async function processDinoImage(buffer) {
  ...
  if (bestIdx >= 0) {
    ...
    // Crop the bounding box of the dino from the original image
    const cropBuf = Buffer.alloc(rect.width * rect.height * 4);
    for (let y = 0; y < rect.height; y++) {
      for (let x = 0; x < rect.width; x++) {
        const srcIdx = ((rect.y + y) * width + (rect.x + x)) * 4;
    ...
    // Resize the crop to match the original species PNG dimensions.
    // The 3D model's UV mapping is based on these dimensions, so the
    // coloring fills the shape instead of appearing as a dino-inside-dino.
    const speciesPng = SPECIES_PNGS[species] || SPECIES_PNGS.trex;
    const speciesMeta = await sharp(speciesPng).metadata();

    const pngBuffer = await sharp(cropBuf, {
  ...
  // No dino found — return the original image as-is
  src.delete(); gray.delete(); binary.delete();
  contours.delete(); hierarchy.delete();
 ...
 * Each coloring page has 1-3 dots: 1=trex, 2=triceratops, 3=brachiosaurus.
 */
