...
/**
 * Ask Haiku to identify the dinosaur species in the drawing.
 * Returns { species }.
 */
async function analyzeWithHaiku(pngBuffer) {
  ...
  const response = await client.messages.create({
    ...
    messages: [{
      ...
      content: [
        ...
        {
          type: 'text',
          text: `This is a kid's dinosaur coloring page (photo or scan). What species is it? T-rex has big head + tiny arms, triceratops has a frill + horns, brachiosaurus has a very long neck. Respond with ONLY one word: trex, triceratops, or brachiosaurus`
        }
      ]
    }]
  });
  ...
  try {
    ...
    for (const s of SPECIES_LIST) {
      if (text.includes(s)) return { species: s };
    }
  } catch (e) {
...
async function processDinoImage(buffer) {
  ...
  const { data, info } = await sharp(buffer)
    .rotate()  // auto-orient using EXIF metadata (critical for phone camera photos)
  ...
  // ── Step 4b: Also erase dots on non-warped path (when grid detection fails) ──
  if (!warpedBuf) {
  ...
  // ── Step 5: Use Haiku for species identification ──
  ...
  const { species } = await analyzeWithHaiku(imageToAnalyze);

  // ── Step 6: Auto-trim to drawing content and resize to species texture ──
  ...
  // Find tightest bounds around non-white content
  ...
  // Add small margin (2%) to avoid clipping strokes
