...
/**
 * Ask Sonnet to identify the dinosaur species by reading the title text on the page.
 * Returns species string (one of SPECIES_LIST).
 */
async function detectSpecies(pngBuffer) {
  ...
  try {
    ...
    const response = await client.messages.create({
      ...
      messages: [{
        ...
        content: [
          ...
          {
            type: 'text',
            text: 'This is a photo of a kids dinosaur coloring page. Read the title text on the page to identify the species. Respond with ONLY one word: trex, triceratops, or brachiosaurus'
          }
        ]
      }]
    });
    ...
    console.error('Species detection error:', e.message);
  }
  return 'trex';
}
...
async function processDinoImage(buffer) {
  ...
  // ── Step 5: Detect species from ORIGINAL photo (Sonnet reads title text) ──
  const originalPng = await sharp(data, { raw: { width, height, channels: 4 } })
    .resize(800, 800, { fit: 'inside' }).png().toBuffer();
  const species = await detectSpecies(originalPng);

  const speciesPng = SPECIES_PNGS[species] || SPECIES_PNGS.trex;
  const speciesMeta = await sharp(speciesPng).metadata();

  // ── Step 6: Auto-trim to dino bounds and resize ──
  // After warp + dot erasure, the only non-white content is the dino drawing.
  // Auto-trim gives us the exact boundary — no AI bbox needed.
  const srcW = warpedBuf ? warpedW : width;
  const srcH = warpedBuf ? warpedH : height;
  const srcBuf = warpedBuf || data;

  let trimTop = srcH, trimBottom = 0, trimLeft = srcW, trimRight = 0;
  ...
  trimBottom = Math.min(srcH - 1, trimBottom + marginY);
  trimLeft = Math.max(0, trimLeft - marginX);
  trimRight = Math.min(srcW - 1, trimRight + marginX);
  ...
  if (tw > 10 && th > 10) {
    ...
    for (let y = 0; y < th; y++) {
      for (let x = 0; x < tw; x++) {
        const srcIdx = ((trimTop + y) * srcW + (trimLeft + x)) * 4;
        const dstIdx = (y * tw + x) * 4;
        trimmedBuf[dstIdx] = srcBuf[srcIdx];
        trimmedBuf[dstIdx + 1] = srcBuf[srcIdx + 1];
        trimmedBuf[dstIdx + 2] = srcBuf[srcIdx + 2];
    ...
    pngBuffer = await sharp(srcBuf, { raw: { width: srcW, height: srcH, channels: 4 } })
      .resize(speciesMeta.width, speciesMeta.height, { fit: 'fill' })
      .png().toBuffer();
  }
