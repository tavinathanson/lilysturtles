...
async function processImage(buffer) {
  ...
  // Pick the largest circle that has a dark (inked) border
  if (circles.cols > 0) {
    const candidates = [];
    ...
    for (const c of candidates) {
      // Sample border to verify it's a dark (black ink) border, not just any edge
      const samples = 72;
      let dark = 0, valid = 0;
      for (let s = 0; s < samples; s++) {
        const angle = (s / samples) * 2 * Math.PI;
        const sx = Math.round(c.cx + c.r * Math.cos(angle));
        const sy = Math.round(c.cy + c.r * Math.sin(angle));
        if (sx < 0 || sx >= width || sy < 0 || sy >= height) continue;
        valid++;
        const pi = (sy * width + sx) * 4;
        if (data[pi] < 80 && data[pi + 1] < 80 && data[pi + 2] < 80) dark++;
      }
      ...
      const insetR = circleR - Math.max(8, Math.round(circleR * 0.07));
